...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		room_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function room_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: ROOM_STORAGE['ajax_nonce'],
		action: 'room_importer_start_import',
	...
	jQuery.post(ROOM_STORAGE['ajax_url'], data, function(response) {
		"use strict";
		var rez = {};
		try {
			rez = JSON.parse(response);
		} catch (e) {
			rez = { error: ROOM_STORAGE['ajax_error']+':<br>'+response };
		...
		if (rez.error === '') {
			...
			// Do next action
			if (idx < steps.length) {
				room_importer_do_action(steps, idx);
			} else {
